...
Usage
-----
Add `api_key: None = Depends(require_api_key)` to any route that needs auth.

Configuration
-------------
Set the API_KEY environment variable to a secret string.
If API_KEY is not set (e.g. local dev without the variable), auth is DISABLED
and a warning is logged at startup — never leave this unset in production.
...
# Read once at import time so the check is a simple string comparison at runtime
